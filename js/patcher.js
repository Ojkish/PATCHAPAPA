...
export class DMXPatcher {
  ...
  init() {
    ...
    const fuseOpts = { keys: ['model','brand'], threshold:0.5, includeMatches:true, minMatchCharLength:1, ignoreLocation:true };
...
onProjectorInput() {
  ...
  const startsWithFilter = projectorLibrary.filter(p => p.model.toLowerCase().startsWith(term));
  ...
  results.forEach(result => {
    ...
    // Mettre en évidence les caractères correspondants dans la marque et le modèle
